...
function buildDigest(name: string, cityLabel: string, cityRuns: RunRow[], unsubUrl: string): { subject: string; html: string } {
...
<tr><td style="padding:32px;">
  ...
  <p style="margin:0 0 20px;font-size:14px;color:#6b7280;">Ciao <strong>${name}</strong>, ecco cosa si corre vicino a te nei prossimi 7 giorni.</p>
...
Deno.serve(async (req) => {
  ...
  // Modalità dry-run: ?dry=1 calcola e riporta i destinatari SENZA inviare email
  const url = new URL(req.url)
  const dryRun = url.searchParams.get('dry') === '1'
  const preview: { email: string; city: string; runs: number }[] = []
  ...
  // Modalità test: ?test=email → invia UN solo digest all'indirizzo indicato,
  // usando la città con più corse. Non tocca gli altri utenti.
  const testEmail = url.searchParams.get('test')
  if (testEmail) {
    const [topCity, topRuns] = [...runsByCity.entries()].sort((a, b) => b[1].length - a[1].length)[0]
    const cityLabel = topRuns[0].city?.trim() ?? topCity
    const unsubUrl = await buildUnsubscribeUrl('test', serviceRoleKey)
    const { subject, html } = buildDigest('runner', cityLabel, topRuns, unsubUrl)
    try {
      await sendEmail(testEmail, subject, html)
      return new Response(JSON.stringify({ test: true, to: testEmail, city: cityLabel, runs: topRuns.length }), { status: 200 })
    } catch (e) {
      return new Response(JSON.stringify({ test: true, error: String(e) }), { status: 500 })
    }
  }
  ...
  for (const p of (profiles ?? [])) {
    ...
    if (dryRun) {
      preview.push({ email: toEmail, city: (p.city as string).trim(), runs: cityRuns.length })
      continue
    }

    const unsubUrl = await buildUnsubscribeUrl(p.id as string, serviceRoleKey)
    const { subject, html } = buildDigest(p.full_name as string, (p.city as string).trim(), cityRuns, unsubUrl)
  ...
  if (dryRun) {
    console.log(`[weekly-digest] DRY RUN — destinatari=${preview.length}`)
    return new Response(JSON.stringify({ dry_run: true, recipients: preview.length, preview }), { status: 200 })
  }
